      ...
      snapshot.forEach((docSnap) => {
        ...
        if (data && data.id) {
          list.push({
            ...
            assignedTurmas: Array.isArray(data.allowedClassIds) ? data.allowedClassIds : (Array.isArray(data.assignedTurmas) ? data.assignedTurmas : []),
            allowedClassIds: Array.isArray(data.allowedClassIds) ? data.allowedClassIds : (Array.isArray(data.assignedTurmas) ? data.assignedTurmas : []),
...
export async function saveUserToFirestore(user: UserProfile) {
  try {
    ...
    await setDoc(docRef, {
      ...
      assignedTurmas: user.allowedClassIds || user.assignedTurmas || [],
      allowedClassIds: user.allowedClassIds || user.assignedTurmas || [],
